...
<body>
	<%
		Object obj1 = session.getAttribute("id");//id 의 value값을 객체형태로 반환
		String id = obj1.toString();
		out.println(id+"<br/>");	
		
		Object obj2 = session.getAttribute("pw");//pw 의 value값을 객체형태로 반환
		...
		Enumeration enumeration1 = session.getAttributeNames();//세션내의 모든 값들을 객체형태로 반환
		while(enumeration1.hasMoreElements()){
			sName = enumeration1.nextElement().toString();//세션객체의 name이 sName에 저장
		...
		while(enumeration2.hasMoreElements()){
			sName = enumeration2.nextElement().toString();//세션객체의 name이 sName에 저장
			sValue = session.getAttribute(sName).toString();//sName의 value값을 sValue에 저장
			out.println("sName :" + sName + "<br/>");
			out.println("sValue :" + sValue + "<br/>");			
		}	
